...
import json
import joblib
from predict import predict_like
...
# =========================================== Chargement du modèle de ML ============================================

@st.cache_resource
def load_model():
    import joblib
    return joblib.load("model_feedback.pkl")
...
if "selected sector" not in st.session_state:
...
@st.cache_data
def get_recommended_jobs(offers_df, user_profile, top_k=10):
    # Récupère les feedbacks utilisateur connus ou les initialise à False
    offers_df = offers_df.copy()
    offers_df["feedback_competence"] = offers_df.get("feedback_competence", False).fillna(False)
    offers_df["feedback_ville"] = offers_df.get("feedback_ville", False).fillna(False)
    offers_df["feedback_secteur"] = offers_df.get("feedback_secteur", False).fillna(False)

    # Conversion des booléens en 0/1 pour compatibilité modèle
    offers_df["feedback_competence"] = offers_df["feedback_competence"].astype(int)
    offers_df["feedback_ville"] = offers_df["feedback_ville"].astype(int)
    offers_df["feedback_secteur"] = offers_df["feedback_secteur"].astype(int)

    # Appel du modèle sur toutes les offres en batch (ultra rapide)
    scores = predict_batch(offers_df)
    offers_df["matching_score"] = scores

    # Tri décroissant et sélection du top_k
    top_offers = offers_df.sort_values("matching_score", ascending=False).head(top_k)
...
# ====================================== Page de création de profil =====================================================
    ...
    with st.form("profile_form"):
        ...
        with col2:
            ...
            sector = st.selectbox("Secteur d'activité", unique_sectors, index=sector_index)
                ...
                try:
                    with engine.begin() as conn:
                        ...
                        if result:
                            ...
                            conn.execute(text("""
                                INSERT INTO users (username, email, location, skills, secteur)
                                VALUES (:username, :email, :location, :skills, :secteur)
                            """), {
                                "username": name,
                                "email": email,
                                "location": location,
                            ...
                            st.success(f"✅ INSERT exécuté avec : {name}, {email}, {location}, {sector}")
...
# ========================== Page de découverte d'emplois ==========================
    ...
    if not st.session_state.profile_complete:
        ...
        # 1. Charger les offres et les interactions du user
        with st.spinner("Chargement des données..."):
            user_id = st.session_state.user_id  # Assuré par la création de profil
            offers_df = pd.read_sql("SELECT * FROM offers", engine)
            interactions_df = pd.read_sql(f"SELECT * FROM interactions WHERE user_id = '{user_id}'", engine)
            interactions_df = interactions_df.rename(columns={"offer_id": "job_id"})

        # 2. Joindre les feedbacks utilisateur
        offers_with_feedback = offers_df.merge(
            interactions_df[["job_id", "feedback_competence", "feedback_ville", "feedback_secteur"]],
            how="left",
            left_on="job_id",
            right_on="job_id"
        )
        offers_with_feedback[["feedback_competence", "feedback_ville", "feedback_secteur"]] = \
            offers_with_feedback[["feedback_competence", "feedback_ville", "feedback_secteur"]].fillna(False)

        # 3. Construire les offres scorées via predict_like
        ...
        # Appel au moteur de recommandations ML
        with st.spinner("🔍 Prédiction des offres en cours..."):
            scored_offers = get_recommended_jobs(offers_df=offers_with_feedback, user_profile=user_profile, top_k=10)

        # Affichage des offres recommandées
        ...
        if total_jobs > 0:
            st.session_state.pending_like_job = jobs[0]
