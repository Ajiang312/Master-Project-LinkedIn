...
villes = [
    "Albi", "Ajaccio", "Agen", "Angoulême", "Aubagne", "Aurillac", "Auxerre", "Bastia", "Bayonne", "Béziers",
    "Blois", "Bourg-en-Bresse", "Brive-la-Gaillarde", "Cahors", "Calais", "Castres", "Charleville-Mézières", 
    "Châteauroux", "Cholet", "Compiègne", "Dax", "Draguignan", "Dunkerque", "Épinal", "Évreux", "Foix", "Fréjus",
    "Gap", "Grasse", "Guéret", "Hyères", "Issy-les-Moulineaux", "La Roche-sur-Yon", "La Seyne-sur-Mer",
    "Laval", "Le Mans", "Le Puy-en-Velay", "Lorient", "Louviers", "Mâcon", "Manosque", "Mende", "Millau",
    "Mont-de-Marsan", "Montélimar", "Narbonne", "Nevers", "Niort", "Orange", "Périgueux", "Rodez", "Romans-sur-Isère",
    "Roanne", "Saint-Brieuc", "Saint-Malo", "Saint-Nazaire", "Sète", "Tarbes", "Thionville", "Tulle", "Vannes"
]

session_count = 0

while True:
    ville = villes[session_count % len(villes)]
    location_param = urllib.parse.quote(ville)
    base_url = f"https://www.linkedin.com/jobs/search?keywords=Ing%C3%A9nieur&location={location_param}"

    session_count += 1
    print(f"\n🔁 Session {session_count} – Scraping pour la ville : {ville}")
    try:
        scrape_linkedin_jobs(base_url)
    except Exception as e:
        print(f"❌ Erreur session {session_count} ({ville}) : {e}")